...
output "ubuntu_deploy_floating_ip" {
  description = "Floating IP for Ubuntu deployment box"
  value       = var.enable_floating_ips ? openstack_networking_floatingip_v2.ubuntu_fip[0].address : ""
}

output "ubuntu_deploy_internal_ip" {
  description = "Internal IP for Ubuntu deployment box"
  value       = openstack_compute_instance_v2.ubuntu_deploy.access_ip_v4
}

output "ubuntu_deploy_name" {
  description = "Name of Ubuntu deployment box"
  value       = openstack_compute_instance_v2.ubuntu_deploy.name
}

output "kali_floating_ip" {
  description = "Floating IP for Kali box"
  value       = var.enable_floating_ips ? openstack_networking_floatingip_v2.kali_fip[0].address : ""
}

output "kali_internal_ip" {
  description = "Internal IP for Kali box"
  value       = openstack_compute_instance_v2.kali.access_ip_v4
}

output "kali_name" {
  description = "Name of Kali box"
  value       = openstack_compute_instance_v2.kali.name
}
...
output "mgmt_network_id" {
  ...
  value       = openstack_networking_network_v2.mgmt_net.id
}

output "mgmt_subnet_cidr" {
  description = "CIDR block for management subnet"
  value       = openstack_networking_subnet_v2.mgmt_subnet.cidr
}
...
output "deployment_summary" {
  ...
  value = {
    ...
    management_network = {
      ...
      ubuntu_ip = "10.200.0.250"
      kali_ip = "10.200.0.251"
    }
  }
}
...
output "all_floating_ips" {
  ...
  value = var.enable_floating_ips ? concat(
    ...
    [openstack_networking_floatingip_v2.ubuntu_fip[0].address],
    [openstack_networking_floatingip_v2.kali_fip[0].address]
  ) : []
}
...
output "ssh_commands" {
  ...
  value = var.enable_floating_ips ? {
    ubuntu = "ssh -i ~/.ssh/homefedora ubuntu@${openstack_networking_floatingip_v2.ubuntu_fip[0].address}"
    kali = "ssh -i ~/.ssh/homefedora kali@${openstack_networking_floatingip_v2.kali_fip[0].address}"
  } : {}
}
